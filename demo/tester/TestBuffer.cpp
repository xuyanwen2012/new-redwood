  ...
  for (int i = 0; i < time_steps; i++) {
    ...
    q.submit([&](auto &h) {
      ...
      auto compute = [=](auto id) {
        for (int k = 0; k < device_steps; k++) device_data[id] += 1.0;
      };
    ...
    for (int i = 0; i < data_size; i += stride) host_data[i] += 1.0;
  }
  put_elapsed_time(it);
...
void device_usm_data(int stride) {
  ...
  for (int i = 0; i < time_steps; i++) {
    ...
    for (int k = 0; k < data_size; k += stride) host_data[k] += 1.0;
  }
  q.wait();
  put_elapsed_time(it);
...
void shared_usm_data(int stride) {
  ...
  for (int i = 0; i < time_steps; i++) {
    ...
    for (int k = 0; k < data_size; k += stride) data[k] += 1.0;
  }
  q.wait();
  put_elapsed_time(it);
...
void serial(int stride) {
  ...
  for (int i = 0; i < time_steps; i++) {
    for (int j = 0; j < data_size; j++) {
      for (int k = 0; k < device_steps; k++) data[j] += 1.0;
    }

    for (int j = 0; j < data_size; j += stride) data[j] += 1.0;
  }
  put_elapsed_time(it);
