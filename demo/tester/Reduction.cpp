  ...
  for (int i = warm_up_token; i < iter; ++i) {
    ...
    q.submit([&](auto &h) {
      ...
      h.parallel_for(num_processing_elements, [=](auto index) {
        ...
        if (end > N) end = N;
        int sum = 0;
        for (size_t i = start; i < end; ++i) sum += buf_acc[i];
    ...
    q.wait();
    {
      ...
      for (int i = 0; i < num_processing_elements; ++i) sum += h_acc[i];
      elapsed += (iter == warm_up_token) ? 0 : timer.Elapsed();
    }
  }
  if (sum == sum_expected)
...
int ComputeTreeReduction2(sycl::queue &q, std::vector<int> &data,
  ...
  for (int i = warm_up_token; i < iter; ++i) {
    ...
    q.wait();
    {
      ...
      sum = 0;
      for (int i = 0; i < num_work_groups2; ++i) sum += h_acc[i];
      elapsed += (iter == warm_up_token) ? 0 : timer.Elapsed();
    }
  }
  elapsed = elapsed / iter;
    ...
    q.submit([&](auto &h) {
      ...
      h.parallel_for(sycl::nd_range<1>{num_work_items, work_group_size},
                     [=](sycl::nd_item<1> item) {
                       ...
                       if (loc_id == 0) accum_acc[group_id] = scratch[0];
                     });
    });
...
int ComputeParallel9(sycl::queue &q, std::vector<int> &data,
  ...
  for (int i = warm_up_token; i < iter; ++i) {
    ...
    q.wait();
    {
      ...
      for (int i = 0; i < num_work_groups; ++i) res += h_acc[i];
      sum = 0;
