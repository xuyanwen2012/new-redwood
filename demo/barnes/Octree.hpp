...
struct Node {
  ...
  _NODISCARD bool IsLeaf() const { return is_leaf; }

  _NODISCARD const PointT* CenterOfMass() const { return &center_of_mass; }
...
class Octree {
 ...
 public:
  ...
  void LoadPayloadRecursive(Node<T>* cur) {
    if (cur->IsLeaf()) {
      ...
      for (auto i = 0u; i < cur->bodies.size(); ++i) {
        ...
        leaf_node_content_table_[offset + i] = data_[idx];
      }

      leaf_node_size_table_[cur->uid] =
          static_cast<unsigned>(cur->bodies.size());
    } else {
